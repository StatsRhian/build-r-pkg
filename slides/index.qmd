...
%%| echo: False
flowchart TD
  start((Current<br>package)) --> editc[[Edit existing code]]
  editc --> loadall["`devtools::load_all()`"]

  start --> doc[[Edit documentation]]
  doc --> devdoc["devtools::document()"]
  
  start --> newfn[[Write new function]]
  newfn --> docload["
    devtools::document()
    devtools::load_all()
  "]

  start --> ready{Ready to<br>publish}
  ready --> check["
    devtools::check()
    devtools::install()
  "]
  check --> publish(Publish app)
```
...
1. Create  and `@export`{.r} function.
